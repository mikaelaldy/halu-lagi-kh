  ...
  return (
    ...
    >
      ...
      <div className="relative flex items-end justify-center w-full h-full pb-0">
        ...
        {product.visualType === 'standee' && (
          <div className="relative flex flex-col items-center">
            {/* Standee Body */}
            <div className="relative w-24 sm:w-32 md:w-40 h-32 sm:h-40 md:h-48 rounded-xl sm:rounded-2xl overflow-hidden shadow-md border-2 border-white/90 bg-amber-50 group-hover:shadow-xl transition-all">
              <img
                src={product.image}
                alt={product.name}
              ...
              <div className="absolute top-1 left-1 sm:top-1.5 sm:left-1.5 bg-[#F6C358] text-[#261A14] font-heading font-black text-[7.5px] sm:text-[9px] px-1 sm:px-1.5 py-0.5 rounded border border-[#261A14]">
                AKRILIK 15cm
              </div>
            </div>
            {/* Acrylic Base Foot sitting on Teal shelf */}
            <div className="w-20 sm:w-26 md:w-32 h-2.5 sm:h-3.5 bg-white/85 border-2 border-[#261A14] rounded-full -mt-1 sm:-mt-1.5 shadow-xs relative z-10 flex items-center justify-center">
              <div className="w-8 sm:w-10 h-0.5 sm:h-1 bg-[#261A14]/30 rounded-full" />
            </div>
          </div>
        )}

        {/* Type: Shaker Keychain */}
        ...
        {product.visualType === 'pouch' && (
          <div className="relative flex flex-col items-center">
            ...
            <div className="w-24 sm:w-32 md:w-38 bg-amber-100 border-2 border-[#261A14] rounded-t-lg py-0.5 flex items-center justify-center relative">
              ...
              <span className="absolute left-1 sm:left-1.5 text-[6.5px] sm:text-[7.5px] font-bold text-[#6D4C41]">10 PCS</span>
            </div>
            {/* Pouch body */}
        ...
        {/* Type: Pin Badge */}
        {product.visualType === 'pin' && (
          <div className="relative flex flex-col items-center">
            <div className="w-22 sm:w-28 md:w-32 h-22 sm:h-28 md:h-32 rounded-full overflow-hidden shadow-md border-2 border-[#261A14] bg-white group-hover:rotate-6 transition-transform">
              <img
                src={product.image}
                alt={product.name}
                className="w-full h-full object-cover"
            ...
            <div className="w-14 sm:w-18 h-2.5 sm:h-3 bg-amber-200 border border-[#261A14] rounded-full -mt-1 shadow-2xs text-[6.5px] sm:text-[7.5px] font-bold text-center text-[#261A14] flex items-center justify-center">
              PIN 44mm
            </div>
          </div>
        )}
        ...
        {/* Type: Washi Tape */}
        {product.visualType === 'tape' && (
          <div className="relative flex flex-col items-center">
            <div className="relative w-22 sm:w-28 md:w-34 h-24 sm:h-32 md:h-36 rounded-lg sm:rounded-xl overflow-hidden shadow-md border-2 border-[#261A14] bg-white">
              <img
                src={product.image}
                alt={product.name}
                className="w-full h-full object-cover"
              />
              <div className="absolute top-1 right-1 bg-amber-400 text-[#261A14] text-[7px] sm:text-[8px] font-black px-1 rounded border border-[#261A14]">
                10 METERS
              </div>
            </div>
        ...
        {/* Type: Canvas Bag */}
        {product.visualType === 'bag' && (
          <div className="relative flex flex-col items-center">
            <div className="relative w-24 sm:w-32 md:w-40 h-28 sm:h-36 md:h-44 rounded-lg sm:rounded-xl overflow-hidden shadow-md border-2 border-[#261A14] bg-white">
              <img
                src={product.image}
                alt={product.name}
                className="w-full h-full object-cover"
              />
              <div className="absolute bottom-1 left-1 sm:bottom-1.5 sm:left-1.5 bg-emerald-700 text-white font-bold text-[7px] sm:text-[8px] px-1 sm:px-1.5 py-0.5 rounded border border-[#261A14]">
                HEAVY CANVAS
              </div>
            </div>
            <div className="w-18 sm:w-26 h-1.5 sm:h-2 bg-[#004D40]/40 rounded-full blur-[1px] -mt-0.5" />
          </div>
        )}

        {/* Fallback */}
        {!['standee', 'shaker', 'pouch', 'box', 'print', 'pin', 'tape', 'bag'].includes(product.visualType || '') && (
